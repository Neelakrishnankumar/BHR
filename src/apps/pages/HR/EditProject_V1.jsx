...
const Editproject_V1 = () => {
  ...
  console.log(
    "🚀 ~ Editproject_V1 ~ staffmappingGetData:",
    staffmappingGetData,
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return TimeTablerows.filter((row) => {
      return (
        ...
        row.area?.some((x) => x.Name?.toLowerCase().includes(search)) ||
        row.day?.some((x) => x.Name?.toLowerCase().includes(search)) ||
        row.slot?.some((x) => x.Name?.toLowerCase().includes(search))
      );
    });
  ...
  const handleDeleteClickTimeTable = (id) => async () => {
    try {
      ...
      const detailPayload = {
        ...
        Day: Array.isArray(row.day) ? row.day.map((d) => d.Name).join(",") : "",
      ...
      const response = await dispatch(
        postData({ accessID: "tr389v1", action: "harddelete", idata }),
      );
  ...
  const processRowUpdateTeach_V1 = async (newRow, oldRow) => {
    ...
    const idata = {
      ...
      ProjectIncharge: data.ProjectIncharge || 0,
      ProjectInchargeName: data.ProjectInchargeName || "",
      ServiceMaintenanceProject: data.ServiceMaintenanceProject,
    ...
    try {
      const response = await dispatch(
        postData({ accessID: "tr389v1", action, idata }),
      );
      ...
      if (response?.payload?.Status === "Y") {
        ...
        dispatch(getFetchData({ accessID: "TR275", get: "get", recID }));
        dispatch(
          staffmappingTeacherget({ ProjectID: recID, CompanyID: CompanyID }),
        );
  ...
  const processRowUpdateUnit = async (newRow, oldRow) => {
    ...
    const payload = {
      data: {
        ...
        SubjectID: newRow.Subject?.RecordID || oldRow.Subject?.RecordID || "",
  ...
  function EditSubjectAutocomplete(props) {
    ...
    const [subjectLookup, setSubjectLookup] = useState(row.Subject || null);
  ...
  function EditAssignedAutocomplete(props) {
    ...
    const departmentId = row?.department?.RecordID || 0;
    const handleChange = async (newValue) => {
      if (!newValue) return;
  ...
  function EditdeptAutocompleteCellTerm(props) {
    const { id, value, field, api, row } = props;
    const [termlookup, setTermlookup] = useState(row.term ? row.term : null);
  ...
  const unitColumns = [
    ...
    },
    {
      ...
      renderEditCell: (params) => <EditSubjectAutocomplete {...params} />,
    },
    {
      field: "Description",
  ...
  const handleProcessSave = async () => {
    ...
    const response = await dispatch(TaskProcess({ data }));
    if (response?.payload?.Status === "Y") {
      toast.success(response?.payload?.Msg || "Processed Successfully");

...
const EditAcademicsForm = () => {
  ...
  useEffect(() => {
    dispatch(
      EventsgetData({ accessID: "TR385", get: "get", recID, Type: "A" }),
    );
  ...
  const Fnsave = async (values, del, override = {}) => {
    ...
    const idata = {
      ...
      StdActivitiesID:
        values?.ApplicableTo?.map((item) => item.RecordID).join(",") || "",
      SubjectID: values?.Subject?.map((item) => item.RecordID).join(",") || "",
  ...
  const getVideoChange = async (event) => {
    ...
    try {
      const fileData = await dispatch(videoUpload({ formData }));
                  ...
                  >
                    ...
                    <Typography
                      ...
                      onClick={() => {
                        // navigate("/Apps/TR243/Party");
                        navigate(
                          `/Apps/Secondarylistview/${params.accessID}/${params.screenName}/${params.leaderID}/Events/${params.secondaryAccessID}/${params.parentID2}/A`,
                          {
                            state: { ...state },
                          },
                        );
                        ...
                        <form onSubmit={handleSubmit}>
                          ...
                          >
                            ...
                            <Box>
                              ...
                              >
                                Manage and track academic events and schedules
                              </Typography>
                            </Box>
                          </Box>
                          ...
                          >
                              ...
                              <EventsmultiSelect
                                ...
                                url={`${listViewurl}?data=${JSON.stringify({
                                  Query: {
                                    ...
                                    ScreenName: "Subjects",
                                    VerticalLicense: "003",
                                    Filter: `parentID='${CompanyID}' AND ProjectID IN (${
                                      values?.ApplicableTo?.map(
                                        (item) => item.RecordID,
                                      )?.join(",") || ""
                                    })`,
                                    Any: "",
                            ...
                            >
                              <LoadingButton
                                ...
                                loading={isLoading}
                                disabled={
                                  mode === "V" || uploadLoading || imageLoading
                                }
                              >
                                Save
                              </LoadingButton>

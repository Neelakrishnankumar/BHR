...
const Editrequests = () => {
  ...
  const [openODPopup, setOpenODPopup] = useState(false); //Onduty
  const [openOTPopup, setOpenOTPopup] = useState(false); // Overtime
  const [openRegPopup, setOpenRegPopup] = useState(false); //Regularization
  const [openExpPopup, setOpenExpPopup] = useState(false); //Expense
  const [openSAPopup, setOpenSAPopup] = useState(false); //Salary Adavance
    ...
    fetch(process.env.PUBLIC_URL + "/validationcms.json")
      ...
      .then((data) => {
        ...
        const schema4 = Yup.object().shape({
          Year: Yup.string().required(data.Leave.Year),
          LeavePart: Yup.string().required(data.Leave.LeavePart),
          leavetype: Yup.object()
            .nullable()
            .required(data.Leave.leavetype),
          FromDate: Yup.string().required(data.Leave.FromDate),
          ToDate: Yup.string().required(data.Leave.ToDate),
          comment: Yup.string().required(data.Leave.comment),
          managerComments: Yup.string().required(data.Leave.managerComments),
          Status: Yup.string().required(data.Leave.Status),
        });
        setValidationSchema4(schema4);
        const schema5 = Yup.object().shape({
          LeavePart: Yup.string().required(data.Onduty.LeavePart),
          ProName: Yup.object()
            .nullable()
            .required(data.Onduty.ProName),
          FromDate: Yup.string().required(data.Onduty.FromDate),
          ToDate: Yup.string().required(data.Onduty.ToDate),
          comment: Yup.string().required(data.Onduty.comment),
          managerComments: Yup.string().required(data.Onduty.managerComments),
          Status: Yup.string().required(data.Onduty.Status),
        });
        setValidationSchema5(schema5);
        const schema6 = Yup.object().shape({
          date: Yup.string().required(data.Salaryadvance.date),
          overhead: Yup.object()
            .nullable()
            .required(data.Salaryadvance.overhead),
          amount: Yup.string().required(data.Salaryadvance.amount),
          comments: Yup.string().required(data.Salaryadvance.comments),
          managerComments: Yup.string().required(data.Salaryadvance.managerComments),
          Status: Yup.string().required(data.Salaryadvance.Status),
        });
        setValidationSchema6(schema6);
        const schema7 = Yup.object().shape({
          date: Yup.string().required(data.Expense.date),
          overhead: Yup.object()
            .nullable()
          ...
          Status: Yup.string().required(data.Expense.Status),
        });
  ...
  const selectCellRowData = ({ rowData, setFieldValue, mode, field }) => {
  ...
  const expensefnSave = async (values, resetForm, del) => {
    ...
    const idata = {
      ...
      OverheadsRecordID: values?.overhead?.RecordID,
      OverHeadsName: values?.overhead?.Name,
  ...
  const salAdFNsave = async (values, resetForm, del) => {
    ...
    const idata = {
      ...
      OverHeadsID: values?.overhead?.RecordID,
      OverHeadsName: values?.overhead?.Name || "",
  ...
  const formSections = [
    { value: "0", label: "Personnel", desc: "Employee basic information", icon: "👤", },
    { value: "11", label: "Permission", desc: "Employee permission requests", icon: "📝", },
    { value: "2", label: "Leave", desc: "Leave application and history", icon: "🌴", },
    { value: "8", label: "On Duty", desc: "Official duty movement", icon: "🚗", },
    { value: "6", label: "Over Time", desc: "Employee overtime details", icon: "⏰", },
    { value: "10", label: "Regularization", desc: "Attendance regularization", icon: "📅" },
    { value: "9", label: "Expense", desc: "Expense reimbursement", icon: "💰", },
    { value: "7", label: "Salary Advance", desc: "Salary advance requests", icon: "💵", },
  ];


  const currentSection =
    formSections.find((item) => String(item.value) === String(show)) || { label: "Personnel" };
                        ...
                        >
                          ...
                          <Box
                            ...
                            sx={{
                              ...
                              "& .even-row": { backgroundColor: "", color: "" },
                          ...
                          {/* ----- LEAVE DETAILS SUMMARY (stays visible on page) ----- */}
                          <Box sx={{ gridColumn: "span 2" }}>
                            ...
                            >
                              ...
                              <Table aria-label="leave details table" size="small">
                                <TableHead>
                                  <TableRow>
                                    <TableCell></TableCell>
                                ...
                                <TableBody>
                                  {leavegridData.LeaveDetailsData.TableData.map((row) => (
                                    <TableRow
                                      key={row.LeaveType}
                                      sx={{
                                        "& td, & th": { paddingTop: "4px", paddingBottom: "4px", height: "30px" },
                                      }}
                                    >
                          ...
                          >
                            ...
                            >
                              <Typography variant="h6">
                                {funMode === "A" ? "Add Leave" : "Edit Leave"}
                              </Typography>
                              <IconButton onClick={() => setOpenADPopup(false)}>
                                <CloseIcon />
                              </IconButton>
                            </DialogTitle>

                            <DialogContent
                              sx={{ pt: 3, "&.MuiDialogContent-root": { paddingTop: "24px" } }}
                            >
                              <Box display="grid" gridTemplateColumns="repeat(2,minmax(0,1fr))" gap={2}>
                                ...
                                <TextField
                                  ...
                                  label={
                                    <span>
                                      Leave Part <span style={{ color: "red", fontSize: "20px" }}>*</span>
                                    </span>
                                  }
                                  ...
                                  name="LeavePart"
                                  select
                                  error={!!touched.LeavePart && !!errors.LeavePart}
                                  helperText={touched.LeavePart && errors.LeavePart}
                                >
                                ...
                                <TextField
                                  ...
                                  label={
                                    <span>
                                      To Date <span style={{ color: "red", fontSize: "20px" }}>*</span>
                                    </span>
                                  }
                                  InputLabelProps={{ shrink: true }}
                                  inputProps={{ min: minDate, max: maxDate }}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add OT" : "Edit OT"}
                            </Typography>
                            <IconButton onClick={() => setOpenOTPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          </DialogTitle>
                      ...
                      >
                        ...
                        >
                          <DataGrid
                            ...
                            pageSize={pageSize6}
                            page={page6}
                            onPageSizeChange={(newPageSize) => setPageSize6(newPageSize)}
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add Salary Advance" : "Edit Salary Advance"}
                            </Typography>
                            <IconButton onClick={() => setOpenSAPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          </DialogTitle>
                          ...
                          >
                            <Box display="grid" gridTemplateColumns="repeat(2,minmax(0,1fr))" gap={2}>
                              ...
                              <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                error={!!touched.referenceifany && !!errors.referenceifany}
                                helperText={touched.referenceifany && errors.referenceifany}
                              />
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add On Duty" : "Edit On Duty"}
                            </Typography>
                            <IconButton onClick={() => setOpenODPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          </DialogTitle>
                          ...
                          >
                            <Box display="grid" gridTemplateColumns="repeat(2,minmax(0,1fr))" gap={2}>
                              ...
                              <TextField
                                ...
                                helperText={touched.ToDate && errors.ToDate}
                                inputProps={{
                                  min: values.FromDate || new Date().toISOString().split("T")[0],
                                }}
                              />
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add Permission" : "Edit Permission"}
                            </Typography>
                            <IconButton onClick={() => setOpenADPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          ...
                          <DialogActions
                            sx={{ borderTop: "1px solid #E5E7EB", p: 2, justifyContent: "space-between" }}
                          >
                            <Button
                              color="error"
                              variant="contained"
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add Expense" : "Edit Expense"}
                            </Typography>
                            <IconButton onClick={() => setOpenExpPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          </DialogTitle>
                          ...
                          >
                            <Box display="grid" gridTemplateColumns="repeat(2,minmax(0,1fr))" gap={2}>
                              ...
                              <TextField
                                ...
                                variant="outlined"
                                type="text"
                                label={<>Manager Comments<span style={{ color: "red", fontSize: "20px" }}>*</span></>}
                                multiline
                                rows={3}
                                value={values.managerComments}
                                id="managerComments"
                              ...
                              <TextField
                                select
                                label={<>Status<span style={{ color: "red", fontSize: "20px" }}>*</span></>}
                                id="Status"
                                name="Status"
                                value={values.Status}
                                onBlur={handleBlur}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Typography variant="h6">
                              {funMode === "A" ? "Add Regularization" : "Edit Regularization"}
                            </Typography>
                            <IconButton onClick={() => setOpenRegPopup(false)}>
                              <CloseIcon />
                            </IconButton>
                          </DialogTitle>

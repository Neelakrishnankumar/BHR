...
const EditAttendanceEntry = () => {
    ...
    const handleUnlockSubmit = async () => {
        ...
        if (response.payload.Status == "Y") {
            ...
            if (payload?.Status === "Y" && Array.isArray(payload?.Data)) {
                // Normalize: any row on leave can never be loaded as Present/shift-adjusted
                const LockgetData = payload.Data.map((value, index) => {
                    const isOnLeave = value.LeaveFlag === "Y";
                    return {
                        ...value,
                        SLNO: index + 1,
                        Present: isOnLeave ? 0 : value.Present,
                        ShiftCheck: isOnLeave ? "N" : value.ShiftCheck,
                        shiftChecked: isOnLeave ? false : value.shiftChecked,
                        ShiftFromTime: isOnLeave ? "" : value.ShiftFromTime,
                        ShiftToTime: isOnLeave ? "" : value.ShiftToTime,
                    };
                });
                setRows(LockgetData);
    ...
    const handleApplyClick = async (values) => {
        const projecID = selectedPro?.map((proj) => proj.RecordID).join(",");
        ...
        try {
            ...
            if (payload?.Status === "Y" && Array.isArray(payload?.Data)) {
                // Normalize: any row on leave can never come in as Present/shift-adjusted
                ...
                setSelectAllPresent(allSelected);
            } else if (payload?.Status === "N") {
                toast.error(payload?.Message || "No data found for the selected date");
                setRows([]);
    ...
    const handleSaveButtonClick = async () => {
        ...
        const formattedData = rows.map((row) => {
            ...
            const isOnLeave = row.LeaveFlag === "Y";
            ...
            return {
                ...
                CompanyID: CompanyID,
                ShiftFromTime: isOnLeave
                    ? ""
                    : isShiftAdjusted
                        ? row.ShiftFromTime
                        : row.ShiftStartTime,

                ShiftToTime: isOnLeave
                    ? ""
                    : isShiftAdjusted
                        ? row.ShiftToTime
                        : row.ShiftEndTime,
    ...
    const handleEditClick = (EmpID) => () => {
        ...
        // Block entering edit mode for any employee currently on leave
        const targetRow = rows.find((r) => r.EmpID === EmpID);
        if (targetRow?.LeaveFlag === "Y") {
            toast.error("This employee is on leave — attendance cannot be edited.");
            return;
        }
    ...
    const processRowUpdate = (newRow, oldRow) => {
        // Never allow a leave-flagged row's edits to be committed
        if (oldRow?.LeaveFlag === "Y") {
            return oldRow;
        }
    ...
    const handleSelectAllPresent = (checked) => {
        ...
        setRows((prevRows) =>
            prevRows.map((row) => ({
                ...
                // LeaveFlag Y should always remain N
                Present: row.LeaveFlag === "Y"
                    ? 0
                    : checked
                        ? 1
                        : 0,
    ...
    // };
    const handlePresentCheckboxChange = (updatedRow) => {
        // Hard block — leave rows can never be marked Present via this path either
        if (updatedRow.LeaveFlag === "Y") return;
    ...
    const AttColumn = [
        {
            ...
            valueGetter: (params) => params.row.SLNO,
        },
        {
            ...
            renderHeader: () => (
                <Box display="flex" alignItems="center" justifyContent="center" gap={0.5}>
                    <Checkbox
                        ...
                        size="small"
                    />
                    <Typography variant="body2" sx={{ color: "white" }}>Present</Typography>
                </Box>
            ),
            renderCell: (params) => (
                <Checkbox
                    checked={
                        params.row.LeaveFlag === "Y"
                            ? false
                            : Boolean(Number(params.row.Present))
                    }
                    disabled={params.row.LeaveFlag === "Y"}
                    onChange={(e) => {
                        // Hard block: leave rows can NEVER become Present = Y,
                        // no matter what the checkbox event says.
                        if (params.row.LeaveFlag === "Y") return;
                        ...
                        setRows((prevRows) =>
                            prevRows.map((r) =>
                                r.SLNO === params.row.SLNO
                                    ? {
                                        ...r,
                                        // Force N (0) for LeaveFlag Y
                                        Present: r.LeaveFlag === "Y"
                                            ? 0
                                            : isChecked ? 1 : 0,
...

        {
            ...
            renderCell: (params) => (
                <IconButton
                    color="primary"
                    // Leave rows can never open the shift popup either
                    disabled={
                        params.row.LeaveFlag === "Y" ||
                        Number(params.row.Present) !== 1
                    }
                    onClick={() => {
                        if (params.row.LeaveFlag === "Y") return;
                        setShiftPopupData({
                                    ...
                                    {({
                                        ...
                                        >
                                            ...
                                            <Box sx={{ gridColumn: "span 4" }}>
                                                ...
                                                >
                                                    <DataGrid
                                                        ...
                                                        // Block cell-level editing (e.g. Remarks) for leave rows
                                                        isCellEditable={(params) => {
                                                            if (params.row.LeaveFlag === "Y") return false;
                                            ...
                                            >
                                                ...
                                                >
                                                    <Chip
                                                        icon={<FollowTheSignsOutlinedIcon color="primary" />}
                                                        label="Leave"
                                                        variant="outlined"
                                                        sx={{
                                                            backgroundColor: "#FFF3CD",

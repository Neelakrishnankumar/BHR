  ...
  const getNetTotal = () => {
    ...
    let base =
      invoice?.BillUnits === "Day"
        ? safe(invoice?.TotalPresentAmount)
        : invoice?.BillUnits === "Hours"
          ? safe(invoice?.Total)
          : safe(invoice?.TotalMonthAmount);

    let cgst =
      invoice?.BillUnits === "Day"
        ? safe(invoice?.DaysCGSTAmt)
        : invoice?.BillUnits === "Hours"
          ? safe(invoice?.CGSTAmount)
          : safe(invoice?.MonthCGSTAmt);

    let sgst =
      invoice?.BillUnits === "Day"
        ? safe(invoice?.DaysSGSTAmt)
        : invoice?.BillUnits === "Hours"
          ? safe(invoice?.SGSTAmount)
          : safe(invoice?.MonthSGSTAmt);
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableHeaderRow}>
            <Text style={[styles.cell, styles.cellSmall, { fontWeight: 'bold' }]}>SL#</Text>
            <Text style={[styles.cell, { width: 180, fontWeight: 'bold' }]}>Description</Text>
            <Text style={[styles.cell, { width: 60, fontWeight: 'bold' }]}>Unit</Text>
            <Text style={[styles.cell, { width: 60, fontWeight: 'bold' }]}>CGST</Text>
            <Text style={[styles.cell, { width: 60, fontWeight: 'bold' }]}>SGST</Text>
          ...
          {items.map((it, idx) => (
            <View style={styles.tableRow} key={idx}>
              <Text style={[styles.cell, styles.cellSmall]}>1</Text>
              <Text style={[styles.cell, { width: 180 }]}>{it.Title}</Text>
              <Text style={[styles.cell, { width: 60 }]}>{it.BillUnits}</Text>
              <Text style={[styles.cell, { width: 60, textAlign: "right" }]}>{it.CGST}%</Text>
              <Text style={[styles.cell, { width: 60, textAlign: "right" }]}>{it.SGST}%</Text>
              <Text style={[styles.cell, { width: 80, textAlign: "right" }]}>{it.BillUnits === "Hours" ? it.TotalHours : it.BillUnits === "Day" ? it.TotalPresentDays : "1"}</Text>
              <Text style={[styles.cell, { width: 80, textAlign: "right" }]}>{it.PerHourRate}</Text>
              <Text style={[styles.cell, { width: 80, textAlign: "right" }]}>{it.BillUnits === "Day"
                ? it.TotalPresentAmount
                : it.BillUnits === "Hours"
                  ? it.Total
                  : it.TotalMonthAmount}</Text>
            </View>
          ))}
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 6 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start' }}>
                <Text style={[styles.small, { fontWeight: 'bold' }]}>HSN Code: </Text>
                <Text style={[styles.small, { fontWeight: 'bold' }]}>{invoice?.HSN}</Text>
              </View>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start' }}>
                <Text style={[styles.small, { fontWeight: 'bold' }]}>Net Total in Words: </Text>
                <Text style={styles.small}>
                  {`${inWords(Math.round(getNetTotal()))} Only`}
                </Text>
                  ...
                  {`${numberToWords(
                    Math.round(
                      (() => {
                        ...
                        // CGST based on BillUnits
                        let cgst =
                          invoice?.BillUnits === "Day"
                            ? Number(invoice?.DaysCGSTAmt || 0)
                            : invoice?.BillUnits === "Hours"
                              ? Number(invoice?.CGSTAmount || 0)
                              : Number(invoice?.MonthCGSTAmt || 0);

                        // SGST based on BillUnits
                        let sgst =
                          invoice?.BillUnits === "Day"
                            ? Number(invoice?.DaysSGSTAmt || 0)
                            : invoice?.BillUnits === "Hours"
                              ? Number(invoice?.SGSTAmount || 0)
                              : Number(invoice?.MonthSGSTAmt || 0);
            ...
            <View style={{ width: 200 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.small}>CGST ({invoice?.CGST}%)</Text>
                {/* <Text style={styles.small}>{invoice?.CGSTAmount}</Text> */}
                <Text style={styles.small}>{invoice?.BillUnits === "Day"
                  ? invoice?.DaysCGSTAmt
                  : invoice?.BillUnits === "Hours"
                    ? invoice?.CGSTAmount
                    : invoice?.MonthCGSTAmt}</Text>

              </View>

              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.small}>SGST ({invoice?.SGST}%)</Text>
                {/* <Text style={styles.small}>{invoice?.SGSTAmount}</Text> */}
                <Text style={styles.small}>{invoice?.BillUnits === "Day"
                  ? invoice?.DaysSGSTAmt
                  : invoice?.BillUnits === "Hours"
                    ? invoice?.CGSTAmount
                    : invoice?.MonthSGSTAmt}</Text>
              </View>
              ...
              {/* <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Text style={[styles.small, { fontWeight: 'bold' }]}>                
                  {(() => {
                    ...
                    return Math.round(Number(net)).toFixed(2);
                  })()}
                </Text>
              </View> */}
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  ...
                  {(() => {
                    ...
                    let sgst =
                      invoice?.BillUnits === "Day"
                        ...
                        : invoice?.BillUnits === "Hours"
                          ? invoice?.SGSTAmount
                          : invoice?.MonthSGSTAmt;

                    // final net
                    let net = Number(base || 0);

                    if (!isGSTZero) {
                      net += Number(cgst || 0) + Number(sgst || 0);
                    }
                ...
                <Text style={[styles.small, { fontWeight: 'bold' }]}>
                  {Math.round(getNetTotal()).toFixed(2)}
                </Text>

...
const mergeProductRows = (rows) => {
  const map = {};
  rows.forEach((row) => {
    const key = [row.Party, row.OROrderDate, row.Price, row.Discount, row.Status].join("|");
    if (!map[key]) {
      map[key] = { ...row, Quantity: Number(row.Quantity || 0), Amount: Number(row.Amount || 0) };
    } else {
      map[key].Quantity += Number(row.Quantity || 0);
      map[key].Amount += Number(row.Amount || 0);
    }
...
// ── NEW: compute grand summary from all filtered rows ──────────────
...
const OrdEnqProductPDF = ({ data = [], filters = {} }) => {
  ...
  // ── compute grand summary once ──
  const { upiTotal, codTotal, pendingTotal } = computeGrandSummary(filteredData);
  const grandTotal = upiTotal + codTotal + pendingTotal;
  ...
  return (
    <Document>
              ...
              >
                ...
                <View style={styles.table} wrap={false}>
                  ...
                  <View style={styles.tableRow}>
                    ...
                    <Text style={[styles.headerCell, { flex: 3.5, borderRightWidth: 1 }]}>Party</Text>
                    <Text style={[styles.headerCell, { flex: 0.6, borderRightWidth: 1 }]}>Qty</Text>
                    <Text style={[styles.headerCell, { flex: 0.8, borderRightWidth: 1 }]}>Rate</Text>
                    <Text style={[styles.headerCell, { flex: 0.9, borderRightWidth: 1 }]}>Disc</Text>
                    <Text style={[styles.headerCell, { flex: 0.9, borderRightWidth: 1 }]}>Value</Text>
                    <Text style={[styles.headerCellStatus, { flex: 1.5, borderRightWidth: 0 }]}>Status</Text>
                  </View>
                  ...
                  {chunk.map((row, i) => (
                    <View key={i} style={styles.tableRow}>
                      ...
                      <Text style={[styles.cell, { flex: 0.6, textAlign: "right", borderRightWidth: 1 }]}>
                        {row.Quantity || ""}
                      </Text>
                  ...
                  {/* PAGE SUBTOTAL ROW */}
                  <View style={[styles.tableRow, { borderBottomWidth: 0 }]}>
                    <Text style={[styles.cell, { flex: 7.3, textAlign: "right", fontWeight: "bold" }]} />
                    <Text style={[styles.cell, { flex: 0.9, textAlign: "right", fontWeight: "bold" }]} />
                    <Text style={[styles.cell, { flex: 1.5, textAlign: "right", fontWeight: "bold" }]}>
                      {pageTotal.toFixed(2)}
                    </Text>
                  </View>
                </View>
      ...
      {/* ══ GRAND SUMMARY PAGE (new, always last) ══ */}
      <Page size="A4" orientation="landscape" style={styles.page}>
        {renderHeader()}

...
const Companychange = () => {
    ...
    const panImageSrc = headerPreview
        ? headerPreview
        : data?.CM_IMAGE
            ? store.getState().globalurl.imageUrl + data.CM_IMAGE
            : null;
    ...
    const getFilepanChange = async (event) => {
        ...
        if (fileData.payload?.Status === "Y") {
            const uploadedLogo = fileData.payload.name;
    ...
    const gstImageSrc = footerPreview
        ? footerPreview
        : data?.CM_GSTIMAGE
            ? store.getState().globalurl.imageUrl + data?.CM_GSTIMAGE
            : null;
    const getFilegstChange = async (event) => {
        const file = event.target.files?.[0];
        if (!file) return;
        ...
        if (fileData.payload?.Status === "Y") {
            const uploadedGst = fileData.payload.name;
    ...
    const saveCompanyDetails = async (values, overrides = {}) => {
        const idata = {
            ...
            Image: overrides.logoimage ?? logoimage,
            GstImage: overrides.gstImage ?? gstImage,
    ...
    const fnSave = async (values) => {
        ...
        if (response.payload.Status === "Y") {
            ...
            navigate("/Apps/ChangeyourPassword_5");
        } else {
                ...
                >
                    {({
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            >
                                <FormControl fullWidth sx={{ gridColumn: "span 2", gap: formGap, flexDirection: "column" }}>
                                    <Box>

...
export function MultiFormikOptimizedselectAutocomplete({
  value = [],
  onChange,
  url,
  ...
  const SELECT_ALL_OPTION = { RecordID: "ALL", Name: "Select All" };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setOptions([SELECT_ALL_OPTION, ...data]);
      } catch (error) {
  ...
  // ✅ Derived values
  const allItems = options.filter((opt) => opt.RecordID !== "ALL");

  const isAllSelected =
    allItems.length > 0 && (value || []).length === allItems.length;

  // ✅ Handle Change
  const handleChange = (event, newValue) => {
    const isSelectAllClicked = newValue.some((item) => item.RecordID === "ALL");

    if (isSelectAllClicked) {
      if (isAllSelected) {
        onChange(event, []);
      } else {
        onChange(event, allItems);
      }
    } else {
      onChange(event, newValue);
    }
  ...
  return (
    <Autocomplete
      ...
      // ✅ Prevent crash + remove "Select All" from chips
      value={(value || []).filter((v) => v.RecordID !== "ALL")}
      ...
      renderOption={(props, option, { selected }) => {
        return (
          <li {...props} style={{ display: "flex", gap: 8, height: 36 }}>
            <Checkbox
              size="small"
              checked={option.RecordID === "ALL" ? isAllSelected : selected}
            />
            {option.Name}
          </li>
        );
...
export const TimeSheetEmployeeautocomplete = ({
  value = null,
  onChange,
  url,
  height = 20,
...
export const Employeeautocomplete_v1 = ({
  value = null,
  onChange,
  url,
  height = 20,
...
export const SettlementSingleSelect = ({
  value = null,
  onChange,
  url,
  height = 15,
...
export const EventsSingleSelect = ({
  value = null,
  onChange,
  url,
  height = 15,
